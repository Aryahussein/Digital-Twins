...
def _mos_current_level1(VI, node_map, comp):
    ...
    # Threshold synonyms
    VTO = params.get("VTO", params.get("VT0", params.get("VTH", params.get("VTH0", 0.7))))
    VTO = float(VTO)
...
def _list_sensitivity_keys(components):
    ...
    for name, comp in components.items():
        if name.startswith("D"):
            ...
            keys += [f"{name}:IS", f"{name}:N", f"{name}:RS", f"{name}:BV", f"{name}:IBV", f"{name}:NBV"]
...
def get_all_sensitivities(components, VI, PsiPhi, node_map, w=0.0, dt=None, Vt=0.02585):
    """
    ...
    for name, comp in components.items():
        ...
        # ---------------- Nonlinear devices: Diode ----------------
            ...
            dId_dRS = _finite_diff(lambda x: Id_with_param("RS", max(0.0, x)), RS0, rel=1e-6, abs_step=1e-12)
            sensitivities[f"{name}:RS"] = -Psi_branch * dId_dRS

            # BV (soft breakdown curve)
            BV0 = float(params.get("BV", 0.0))
            dId_dBV = _finite_diff(lambda x: Id_with_param("BV", max(0.0, x)), BV0, rel=1e-6, abs_step=1e-6)
            sensitivities[f"{name}:BV"] = -Psi_branch * dId_dBV

            # IBV / NBV
            IBV0 = float(params.get("IBV", 1e-3))
            dId_dIBV = _finite_diff(lambda x: Id_with_param("IBV", max(0.0, x)), IBV0, rel=1e-6, abs_step=1e-12)
            sensitivities[f"{name}:IBV"] = -Psi_branch * dId_dIBV

            NBV0 = float(params.get("NBV", 1.0))
            dId_dNBV = _finite_diff(lambda x: Id_with_param("NBV", max(1e-6, x)), NBV0, rel=1e-6, abs_step=1e-9)
        ...
        # ---------------- Nonlinear devices: MOSFET ----------------
            ...
            if "KP" in params:
                ...
                dId_dMU = _finite_diff(lambda x: Id_mos_with_param("MU", x), mu0, rel=1e-6, abs_step=1e-12)
                dId_dCOX = _finite_diff(lambda x: Id_mos_with_param("C_OX", x), cox0, rel=1e-6, abs_step=1e-12)
...
def compute_step_sensitivities(lu, VI, components, node_map, output_nodes=None, w=0.0, dt=None, Vt=0.02585):
    ...
    for out_node in output_nodes:
        ...
        comp_sens = get_all_sensitivities(components, VI, PsiPhi, node_map, w=w, dt=dt, Vt=Vt)
...
def aggregate_sweep_sensitivities(components, node_map, analyses,
                                  ...
                                  list_of_lus=None, VI_list=None, freq_list=None, dt=None, Vt=0.02585):
    ...
    sensitivity_dict = {node: {k: [] for k in all_keys} for node in target_nodes}
        ...
        for i in range(len(list_of_lus)):
            if is_ac and freq_list is not None:
                ...
                step_sens = compute_step_sensitivities(
                    list_of_lus[i], VI_list[i], components, node_map, target_nodes, w=w_step, Vt=Vt
                )
            else:
                step_sens = compute_step_sensitivities(
    ...
    for key, sens in sensitivities.items():
        ...
        if ":" in key:
            ...
            if dev.startswith("D"):
                ...
                if pname == "VTO":
                    base = float(params.get("VTO", params.get("VT0", params.get("VTH", params.get("VTH0", 0.0)))))
                elif pname == "KP":
                    base = float(params.get("KP", 0.0))
                elif pname == "MU":
                    base = float(params.get("MU", params.get("UO", params.get("U0", 0.0))))
                elif pname == "COX":
