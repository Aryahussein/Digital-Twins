    ...
    def get_sensitivity(self, node, param, output_format="integrated"):
        ...
        if self.type == ".TRAN":
            ...
            if exact_key not in time_series_dict:
                raise KeyError(f"No sensitivity data calculated for output node '{exact_key}'.")

            if param not in time_series_dict[exact_key]:
                raise KeyError(f"Parameter '{param}' not found in sensitivities. Known parameters: {list(time_series_dict[exact_key].keys())}")
