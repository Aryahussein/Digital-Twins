...
class SensitivityData:
    ...
    def __init__(self, sweep_axis, param_names, output_nodes, domain="static"):
        ...
        self.data = np.zeros(
            (len(param_names), len(output_nodes), len(self.sweep_axis)), dtype=dtype
        )
    ...
    def print_matrix_at_step(self, step_idx, output_node, sens_parameter):
        ...
        Args:
            ...
            output_node (str): The target output node.
            params (list[str]): The list of parameters to display.
        """
...
class SimulationResult:
    ...
    def __init__(self, analysis_type, sweep_axis, VI_matrix, node_map, domain, step, list_of_lus):
        ...
        self.domain = domain                 # Simulator adds this later
        self.step = step                          # Simulator adds this later
        self.list_of_lus = list_of_lus             # Simulator adds this later
    ...
    def get_vhats(self, node):
        ...
        for i, v_hat in enumerate(adjoint_history[:5]):
            print(f"  Step {i}: {v_hat[idx]}")
        adjoint_matrix = np.array(adjoint_history)
        return adjoint_matrix[:, idx]
