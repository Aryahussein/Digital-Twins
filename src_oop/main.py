...
if __name__ == "__main__":
    # --- Configuration ---
    netlist = "1T1C_dram_cell"  # nmos_inverter, rc_lowpass, etc.
    file_path = f"../testfiles/{netlist}.txt"
    target_node = "SN"
    target_component = "M1_VTO"
    radiation = True
    ...
    circuit, result = run_simulation_core(
        ...
        sensitivity=True,
        keep_lus=True,
        global_adjoint=True,  # Ensure we calculate the global backward integrals for TRAN
    )
    ...
    if result.type == ".TRAN":
        ...
        if calculated_params:
            ...
            for param in calculated_params:
                ...
                if result.global_sensitivities:
                    val = result.global_sensitivities["Integrated_Transient"][
                        target_node
                    ].get(param, 0.0)
        ...
        for node in output_nodes:
            ...
            plot_all_faults(
                ...
                delta_F,
                folder="../figures/fault",
                name=f"{netlist}_all_faults_{node}",
            )
        ...
        if target_component in calculated_params:
            if radiation:
                ...
                plot_transient_sensitivity_to_radiation(
                    ...
                    folder="../figures/tran",
                    name=f"{netlist}_tran_sens_radiated",
                )
    ...
    # ==========================================
        ...
        delta_F = 0.1
        for node in output_nodes:
            thresholds = compute_fault_thresholds(
                circuit, result, node, delta_F=delta_F
            )
            print_threshold_table(thresholds)
