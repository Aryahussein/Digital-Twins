...
def plot_ac_sensitivity(freqs, VI, sensitivities, node_map, output_node,
                        target_component=None, folder="../figures/ac", name="ac_sens",
                        top_n=30):
    import os
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Pick a default output node if None
    if output_node is None:
        numeric_nodes = [k for k in node_map.keys() if isinstance(k, int)]
        output_node = max(numeric_nodes) if numeric_nodes else 0

    # Guard: must have sensitivity data
    if sensitivities is None or output_node not in sensitivities:
        print("No sensitivity data available to plot.")
        return

    # If user did not specify a component, plot multiple curves (top N by peak magnitude)
    if target_component is None:
        comp_dict = sensitivities[output_node]  # {comp_key: array_over_freq}
        keys = list(comp_dict.keys())
        if len(keys) == 0:
            print("Sensitivity dictionary is empty.")
            return

        # Rank by max magnitude across sweep
        ranked = sorted(
            keys,
            key=lambda k: np.nanmax(np.abs(comp_dict[k])) if np.size(comp_dict[k]) else 0.0,
            reverse=True
        )
        keys_to_plot = ranked[:top_n]

        plt.figure()
        for k in keys_to_plot:
            y = np.abs(comp_dict[k])
            if y.size == 0:
                continue
            plt.loglog(freqs, y, label=k)

        plt.xlabel("Frequency (Hz)")
        plt.ylabel("|dOutput/dp|")
        plt.title(f"AC Sensitivity at output node {output_node} (top {len(keys_to_plot)})")
        plt.grid(True, which="both", ls="--", alpha=0.3)
        plt.legend(fontsize=8, ncol=2)
        plt.tight_layout()
        plt.savefig(f"{folder}/{name}.png", dpi=600)
        plt.close()
        return

    # Otherwise: plot one requested component
    if target_component not in sensitivities[output_node]:
        available = list(sensitivities[output_node].keys())
        print(f"Component '{target_component}' not found for node {output_node}.")
        print(f"Available keys (first 30): {available[:30]}")
        return

    sens_mags = np.abs(sensitivities[output_node][target_component])

    plt.figure()
    plt.loglog(freqs, sens_mags)
    plt.xlabel("Frequency (Hz)")
    plt.ylabel(f"|dOutput/d({target_component})|")
    plt.title(f"AC Sensitivity at output node {output_node} w.r.t {target_component}")
    plt.grid(True, which="both", ls="--", alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{folder}/{name}.png", dpi=600)
    plt.close()
