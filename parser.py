...
class NetlistParser:
    def __init__(self):
        ...
        self.dispatch_registry = {
            ...
            'G': self._parse_vccs,
            'O': self._parse_opamp,   # <-- add
        }
    def _parse_opamp(self, tokens, name, type_char):
        if len(tokens) < 5:
            self._throw_error(f"Malformed opamp '{name}'. Format: Oname nout nref nplus nminus [A or A=...]")
    ...
    def _parse_tran_func(self, func_str):
        ...
        if name == "PULSE":
            ...
            result["PHASE"] = np.radians(result["PHASE"]) # Convert degrees to rads for Numpy
            return result
        return None
