...
def parse_tran_func(func_str):
    ...
    if name == "PULSE":
        ...
        result = {"type": "SIN", **dict(zip(keys, args + [0]*(5-len(args))))}
        # SPICE convention: phase is in degrees, convert to radians for np.sin
        result["PHASE"] = np.radians(result["PHASE"])
        return result
    elif name == "COS":
        keys = ["VOFF", "VAMP", "FREQ", "TD", "PHASE"]
        result = {"type": "COS", **dict(zip(keys, args + [0]*(5-len(args))))}
        result["PHASE"] = np.radians(result["PHASE"])
        return result
    return None
...
def parse_netlist(file_path):
    ...
    for line in full_lines:
        ...
        if cmd.startswith('.'):
            if cmd == ".MODEL":
                ...
                # Safely extract all possible arguments
                tstep = parse_value(tokens[1]) if len(tokens) > 1 else 0.0
                tstop = parse_value(tokens[2]) if len(tokens) > 2 else 0.0
                tmax  = parse_value(tokens[4]) if len(tokens) > 4 else 0.0
                
                # If TSTEP is 0 (LTspice behavior), use TMAX. 
                # If both are 0, fallback to 1000 steps to prevent crashes.
                dt = tstep if tstep > 0 else (tmax if tmax > 0 else (tstop / 1000.0 if tstop > 0 else 1e-5))
                
                analyses[cmd] = {"step": dt, "stop": tstop}
            elif cmd == ".AC":
    ...
    # Default to DC Operating Point if no analysis is specified
    if not analyses:
        analyses[".OP"] = {}
