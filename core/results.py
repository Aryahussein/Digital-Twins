    ...
    def get_sensitivity(self, node, param, output_format="integrated"):
        ...
        Args:
            ...
            output_format (str, optional): Only applies to TRAN analysis.
                - "integrated": Returns the final scalar sensitivity.
                    Final-time mode:   ∂v(T)/∂p  (scalar)
                    Time-series mode:  ∂v(t_last)/∂p  (scalar, == last sample)
                - "series": Returns the sensitivity array.
                    True ∂V(t_k)/∂p at each simulation timestep t_k.
                    Same length as sweep_axis — plot directly against time.
                - "accumulator": Cumulative sum of the series array.
