...
class ACEngine:
    ...
    def _solve_single_point(self, w, Y_small_signal_base):
        ...
        """
        # 1. Start with the pre-linearized matrix
        ...
        # 4. Convert to CSC right before solving
    ...
    def run(self, Y_base_lil, VI_dc, start_freq, stop_freq, points, sweep_type="DEC", keep_lus=False):
        ...
        if sweep_type.upper() == "LIN":
            ...
            if start_freq <= 0:
                raise ValueError(
                    f"AC start frequency must be > 0 for logarithmic sweep "
                    f"(got {start_freq} Hz). Use a small positive value like 0.01 Hz, "
                    f"or use a LIN sweep type for linear spacing."
                )
