...
def stamp_diode(Y, sources, comp, node_map, name, p_V_guess, V_guess):
    ...
    # Some decks store diode "value" as IS if no model is used
    if "value" in comp:
    ...
    # ---- Node voltages & limiting (outer diode voltage) ----
    ...
    # Limit the step on terminal voltage to help Newton
    vd_lim = pnjlim(vd_k, p_vd_k, 1.0)

    # ---- Junction current I(vj) and derivative dI/dVj ----
    # If RS>0, junction voltage solves: vj = vd_lim - RS * I(vj)

    def I_and_g(vj):
        # Forward exponential (clamped to avoid overflow)
        arg_f = np.clip(vj / Vte, -50.0, 50.0)
        ef = np.exp(arg_f)
        Ifwd = Is * (ef - 1.0)
        gfwd = (Is / Vte) * ef

        # Soft breakdown for vj < -BV
        Ibr = 0.0
        gbr = 0.0
        if BV > 0.0 and vj < -BV:
            x = -(vj + BV)  # >= 0 in breakdown
    ...
    # Solve implicit RS equation for vj (small fixed-point loop)
    vj = vd_lim
    if RS > 0.0:
        for _ in range(8):
            Ij, _ = I_and_g(vj)
    ...
    # Effective terminal conductance with series resistance
    gd = g0 / (1.0 + g0 * RS) if RS > 0.0 else g0

    # Linearized companion: I ≈ gd*Vd + Ieq
    ieq = Id - gd * vd_lim

    # ---- Stamp into Y like a conductance between nodes ----
    if idx1 is not None:
        Y[idx1, idx1] += gd
        if idx2 is not None:
            Y[idx1, idx2] -= gd
...
def stamp_mosfet(Y, sources, comp, node_map, name, p_V_guess, V_guess):
    ...
    m_type = comp["model_type"]  # e.g., "NMOS"

    # ---- Threshold (accept common synonyms) ----
    VTO = params.get("VTO", params.get("VT0", params.get("VTH", params.get("VTH0"))))
    if VTO is None:
        raise ValueError("No threshold specified for MOSFET model (expected VTO/VT0/VTH/VTH0).")
    VTO = float(VTO)

    # ---- Geometry (allow missing W/L for robustness) ----
    W = float(inst_params.get("W", 1.0))
#    L = float(inst_params["L"])
    L = float(inst_params.get("L", 1.0))
    L = max(L, 1e-12)

    # ---- Gain parameter: KP OR (MU/UO)*(C_OX/COX) ----
    if "KP" in params:
        KP = float(params["KP"])
    else:
        ...
        if mu is None or cox is None:
            raise ValueError("No KP or (MU/UO and C_OX/COX) specified for MOSFET model!")
...
def stamp_opamp(Y, sources, comp, node_map, name, p_V_guess=None, V_guess=None):
    ...
    using the branch variable index node_map[name].

    Gain ramping:
        Aeff = comp["value"] * comp.get("_gain_scale", 1.0)
    """
