...
def stamp_ideal_opamp(Y, sources, comp, node_map, name):
    n_out, n2 = comp["n1"], comp["n2"]
    n_plus, n_min = comp["n3"], comp["n4"]
    gain = comp["value"]
    inverse_gain = 1 / gain

    idx = node_map[name]
    p, q = get_idx(n_out, node_map), get_idx(n2, node_map)
    k, l = get_idx(n_plus, node_map), get_idx(n_min, node_map)

    if q is not None:
        raise ValueError("Unexpected argument: The output voltage should be referenced to ground (GND)!")
    if p is not None:
        Y[p, idx] += 1                      # The current flows in or out of the output node Vp.
        Y[idx, p] -= inverse_gain           # In an ideal opamp this goes to zero.
    if k is not None:
        Y[idx, k] += 1                      # KVL equation
    if l is not None:
        Y[idx, l] += -1                     # KVL equation
    sources[idx] = 0                        # No current between the input voltages through the opamp.
...
# =============================================================================
# NON-LINEAR STAMPS (Diodes, Op-Amp)
# =============================================================================
