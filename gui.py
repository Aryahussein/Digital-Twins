...
class CircuitSimulatorGUI:
    def __init__(self, root, simulation_callback):
        ...
        self.btn_load = tk.Button(
            control_frame, text="Load Netlist", command=self.load_file, width=15
        )
        ...
        tk.Label(
            selector_frame, text="Visible Nodes:", bg="#f0f0f0", font=("Arial", 10, "bold")
        ).pack(pady=5)
        self.node_listbox = tk.Listbox(
            selector_frame, selectmode=tk.MULTIPLE, font=("Arial", 10), height=15
        )
    ...
    def _simulation_error(self, error):
        ...
        messagebox.showerror(
            "Simulation Error", f"Simulation failed:\n\n{str(error)}"
        )
    ...
    def _simulation_complete(self):
        ...
        if not analyses:
            messagebox.showwarning(
                "Missing Analysis",
                "No analysis command (.OP, .AC, or .TRAN) was found in the netlist.",
            )
            return
        ...
        for i in sorted_indices:
            ...
            display_name = (
                f"V({node_name})" if isinstance(node_name, int) else f"I({node_name})"
            )
    ...
    def update_data_tab_op(self):
        """Displays DC Operating point, sensitivity, and adjoint analysis results."""
        self.data_text.delete(1.0, tk.END)
        VI = self.sim_data["VI"]
        node_map = self.sim_data["node_map"]
        sens_data = self.sim_data.get("sens_post_proc") or {}
        idx_to_node = {v: k for k, v in node_map.items()}
        ...
        # Adjoint application results
        adj = self.sim_data.get("adjoint")
        if adj:
            res_str += "\n" + "=" * 50 + "\n"
            res_str += "ADJOINT APPLICATIONS\n"
            res_str += "=" * 50 + "\n"

            if "noise" in adj:
                n = adj["noise"]
                res_str += f"\nNoise Analysis:\n"
                res_str += f"  Total: {n['total_Vrms_per_rtHz']:.4e} V/√Hz\n"
                for comp, val in sorted(
                    n["contributions"].items(), key=lambda x: -x[1]
                ):
                    if val > 0:
                        import numpy as np
                        pct = val / max(n["total_V2_per_Hz"], 1e-30) * 100
                        res_str += (
            ...
            if "yield" in adj:
                y = adj["yield"]
                res_str += f"\nYield Estimation:\n"
                res_str += f"  σ_output = {y['sigma_output']:.4e}\n"
                res_str += f"  Yield    = {y['yield_estimate']*100:.4f}%\n"
                res_str += f"  Cpk      = {y['cpk']:.2f}\n"
                if y["top_contributors"]:
                    res_str += f"  Top contributors:\n"
                    for name, pct in y["top_contributors"][:5]:
                        res_str += f"    {name}: {pct:.1f}% of σ\n"
            ...
            if "fault_dictionary" in adj:
                ...
                for comp_name, entries in list(fd.items())[:6]:
                    res_str += f"  {comp_name}:\n"
                    for e in entries:
                        res_str += f"    {e['fault']:>30s} → ΔV = {e['delta_output']:+.4e}\n"

        self.data_text.insert(tk.END, res_str)

    def update_plot(self):
        """Dynamically draws the Bode, Transient, and Sensitivity Plots."""
        if self.sim_data is None or self.sim_data["VI"] is None:
            return
        ...
        analyses = self.sim_data["analyses"]
        node_map = self.sim_data["node_map"]
        # FIX: Safely handle None sensitivity data
        sens_data = self.sim_data.get("sens_post_proc") or {}
        ...
        idx_to_node = {v: k for k, v in node_map.items()}
        sorted_sim_indices = sorted(
            idx_to_node.keys(), key=lambda i: str(idx_to_node[i])
        )
        ...
        if ".AC" in analyses:
            ...
            for listbox_idx in selected_indices:
                ...
                if (
                    show_sens
                    and node_name in sens_data
                    and target_comp in sens_data[node_name]
                ):
                    sens_vals = sens_data[node_name][target_comp]
                    if np.size(sens_vals) > 0:
                        sens_mag = np.abs(sens_vals)
                        ax3.semilogx(
                            x_axis,
                            sens_mag,
                            label=f"d({label})/d{target_comp}",
                            lw=2,
                            linestyle="--",
                        )
